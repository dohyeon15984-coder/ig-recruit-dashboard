...
const { CATEGORIES: DEFAULT_CATEGORIES } = require('./categories');
...
const POST_COLLAB_FILE = path.join(DATA_DIR, 'postCollabInfo.json');
const MANUAL_POSTS_FILE = path.join(DATA_DIR, 'manualPosts.json');
const OVERRIDABLE_POST_FIELDS = ['reach', 'views', 'like_count', 'comments_count', 'saved', 'shares', 'reposts', 'profile_visits', 'follows'];
...
// 실제 동기화된 게시물뿐 아니라 수동으로 추가한 게시물(manualPosts.json)도 카테고리 태깅
// 대상이 될 수 있어, 둘 중 어느 목록에 있는지 찾아서 그 목록에 저장한다.
function setPostCategory(postId, category) {
  ...
  const manualPosts = loadManualPosts();
  const manualPost = manualPosts.find((p) => p.id === postId);
  if (manualPost) {
    manualPost.category = category;
    writeJson(MANUAL_POSTS_FILE, manualPosts);
    return manualPost;
  }
...
// 게시물 카테고리 태그 목록. 기본값은 lib/categories.js 시드값이며, 사용자가 추가/이름변경/삭제하면
// data/categories.json에 저장된다 (다른 데이터 파일들처럼 Render 재배포/스핀다운 시 함께 초기화되므로
// push-to-remote.js / admin/import 동기화 대상에도 포함시켜야 한다).
...
function addCategory(name) {
  ...
  if (categories.includes(trimmed)) return categories;
  const updated = [...categories, trimmed];
  saveCategories(updated);
  return updated;
}

// 이름을 바꾸면 그 카테고리로 태깅된 기존 게시물들도 함께 갱신한다.
function renameCategory(oldName, newName) {
  const trimmed = (newName || '').trim();
  if (!trimmed) throw new Error('새 카테고리 이름을 입력해주세요.');
  const categories = loadCategories();
  if (!categories.includes(oldName)) throw new Error('존재하지 않는 카테고리예요.');

  const updated = [...new Set(categories.map((c) => (c === oldName ? trimmed : c)))];
  saveCategories(updated);
  ...
  if (trimmed !== oldName) {
    ...
    if (changed) writeJson(POSTS_FILE, posts);
  }

  return updated;
}

// 삭제된 카테고리로 태깅되어 있던 게시물은 미지정 상태로 되돌린다.
...
// 공동 게시물(콜라보) 여부/협업 계정 수동 기록. Meta API의 /tags 엔드포인트가 권한 문제로 막혀
// 자동 감지가 불가능해(2026-09-17 확인), 사용자가 게시물 상세 모달에서 직접 표시해두는 용도.
function loadPostCollabInfo() {
  return readJson(POST_COLLAB_FILE, {});
}

function savePostCollabInfo(postId, { isCollab, partner }) {
  const info = loadPostCollabInfo();
  if (isCollab) {
    info[postId] = { isCollab: true, partner: (partner || '').trim() };
  } else {
    delete info[postId];
  }
  writeJson(POST_COLLAB_FILE, info);
  return info;
}

function clearPostCollabInfo(postId) {
  const info = loadPostCollabInfo();
  delete info[postId];
  writeJson(POST_COLLAB_FILE, info);
  return info;
...
function applyPostCollabInfo(posts) {
  const info = loadPostCollabInfo();
  if (!Object.keys(info).length) return posts;
  return posts.map((p) => {
    const c = info[p.id];
    if (!c) return p;
    return { ...p, is_collab: c.isCollab, collab_partner: c.partner || null };
  });
}

// Meta API가 원작성자가 아닌 계정의 콜라보 게시물을 /media에 아예 내려주지 않아
// (/tags 권한도 막혀있어 자동 감지 불가), 동기화로는 절대 잡히지 않는 게시물을 사용자가
// 직접 한 건씩 입력해서 넣을 수 있게 하는 용도. 실제 게시물과 동일하게 표/차트/요약에 합쳐진다.
function loadManualPosts() {
  return readJson(MANUAL_POSTS_FILE, []);
}

function saveManualPost(post) {
  const posts = loadManualPosts();
  const id = post.id || `manual_${Date.now()}_${Math.round(Math.random() * 1000)}`;
  const record = { ...post, id, manual: true };
  const filtered = posts.filter((p) => p.id !== id);
  filtered.push(record);
  writeJson(MANUAL_POSTS_FILE, filtered);
  return record;
}

function deleteManualPost(id) {
  const posts = loadManualPosts().filter((p) => p.id !== id);
  writeJson(MANUAL_POSTS_FILE, posts);
  return posts;
}
